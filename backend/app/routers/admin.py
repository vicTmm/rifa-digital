...
from backend.app.schemas.financial import AdminStatsResponse, WithdrawalResponse
...
def list_withdrawals(current_admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    return db.query(WithdrawalRequest).order_by(WithdrawalRequest.requested_at.desc()).all()
